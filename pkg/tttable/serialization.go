...
// or marshaling fails.
func Serialize(t *Table, method Serialization) ([]byte, error) {
	switch method {
	case ToJSON:
		return t.toJSON()
	case ToJSONIndent:
...
func Deserialize(data []byte) (*Table, error) {
	...
	return nil, fmt.Errorf("failed to deserialize data as any supported format:\n  - %s",
		strings.Join(errs, "\n  - "))
}
...
func Load(path string) (*Table, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to deserialize file %q: %w", path, err)
	}
	return table, nil
}
